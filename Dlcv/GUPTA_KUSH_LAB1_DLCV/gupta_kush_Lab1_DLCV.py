...
def computeMSE(prev, curr):
    #mse = 0
    current = curr.astype(np.int32)
    previous = prev.astype(np.int32)
    diff = np.square(previous - current)
...
def computeEntropy(img):
    ...
    marg_dist = np.histogram(img, density=True)[0]
    marg_dist = list(filter(lambda p: p > 0, np.ravel(marg_dist)))
    marg_dist=np.array(marg_dist).astype(np.float32)
    ent = -np.sum(np.multiply(marg_dist, np.log2(marg_dist)))
    #print(f'ent:',{ent})
    return ent
    
def computeErrorImage(im1, im2):
    error_img= im2-im1
    res=np.clip(error_img+128,0,255)
    return res
...
def computeGME(flow):
    ...
    targetPts=target.reshape((h*w,2))

    homography,mask=cv2.findHomography(srcPts,targetPts,cv2.RANSAC)
    #   see https://docs.opencv.org/4.5.3/d9/d0c/group__calib3d.html#gafd3ef89257e27d5235f4467cbb1b6a63
    tfm=cv2.perspectiveTransform(src,homography)
...
def computeGMEError(flow, gme):
    gme_error1= np.square(flow[:,:,0]-gme[:,:,0])
    gme_error2= np.square(flow[:,:,1]-gme[:,:,1])
    err = np.sqrt( gme_error1+ gme_error2 )
    return err
